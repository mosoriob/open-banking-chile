...
import { deduplicateMovements, dropDuplicateInterestLines, closePopups, delay, normalizeDate, parseChileanAmount } from "../utils.js";
...
/**
 * Detects the "tasa int. X%" interest-rate sub-line of an installment purchase.
 * Whether it's a duplicate (Santander repeats the installment amount on a paired
 * purchase line) or the only record of a real 0% installment is decided later by
 * dropDuplicateInterestLines, which pairs it against its purchase line.
 */
...
export function normalizeSantanderUnbilledApiMovements(captures: unknown[]): BankMovement[] {
  ...
  return dropDuplicateInterestLines(movements);
}
